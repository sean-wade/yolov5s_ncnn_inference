...
class Focus(nn.Module):
    ...
    def forward(self, x):
        y  =   self.conv(torch.cat([torch.nn.functional.interpolate(x, scale_factor=0.5, recompute_scale_factor=True),
                                    torch.nn.functional.interpolate(x, scale_factor=0.5, recompute_scale_factor=True),
                                    torch.nn.functional.interpolate(x, scale_factor=0.5, recompute_scale_factor=True),
                                    torch.nn.functional.interpolate(x, scale_factor=0.5, recompute_scale_factor=True)], 1))
        #print(" ======== >>>>>>>> Focus  : ", y.shape)
        return y

'''
UserWarning: 
The default behavior for interpolate/upsample with float scale_factor changed in 1.6.0 to align with other frameworks/libraries, 

and uses scale_factor directly, instead of relying on the computed output size. 
